...
namespace App\Http\Services\reportes_sofia;
...
class ReportSofiaService
{
    ...
    {
        $user = DB::table('usuarios', 'us')->select(['us_cent_id_fk as userCenter'])->where('us.us_id', $idUser)->first();
    ...
    public function getReportProductiveStage($tokenCodes, $centerCode)
    {
        $sofiaData = DB::connection("oracle_service_SOFIA")->select("SELECT DISTINCT
    ...
    public function getReportAlternativeProductiveStageRegistred($tokenCodes)
    {
        $sofiaData = DB::connection("oracle_service_SOFIA")->select("SELECT DISTINCT
